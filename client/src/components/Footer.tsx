...
import { Mail, Twitter, Instagram, Music, MapPin } from "lucide-react";
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div className="col-span-1 md:col-span-2">
            ...
            {/* Offices */}
            <div className="flex flex-wrap gap-3 mb-6 font-mono text-gray-400 text-sm">
              {offices.map((city, index) => (
                <span key={index} className="flex items-center gap-1">
                  <MapPin size={10} className="text-[#CCFF00]" /> {city}
                </span>
              ))}
            </div>
            ...
            <p className="font-mono text-gray-400 mb-6 max-w-md">
              {isSpanish 
                ? "La agencia digital de nueva generación. Transformamos tus ideas en productos excepcionales gracias a tecnologías de vanguardia y una red mundial de expertos."
          ...
          {/* Links */}
          <div>
            <h3 className="font-bold uppercase text-[#CCFF00] mb-6">
              {isSpanish ? "Navegación" : "Navigation"}
            </h3>
            <ul className="space-y-4 font-mono">
              <li><Link href="/"><span className="hover:text-[#CCFF00] cursor-pointer">{t('footer.home')}</span></Link></li>
              <li><Link href="/method"><span className="hover:text-[#CCFF00] cursor-pointer">{t('footer.method')}</span></Link></li>
              <li><Link href="/services"><span className="hover:text-[#CCFF00] cursor-pointer">{t('footer.services')}</span></Link></li>
              <li><Link href="/blog"><span className="hover:text-[#CCFF00] cursor-pointer">{t('footer.resources')}</span></Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 font-mono">
              <li><Link href="/contact"><span className="hover:text-[#CCFF00] cursor-pointer">{t('footer.contact')}</span></Link></li>
              <li><a href="mailto:Techflow@outlook.fr" className="hover:text-[#CCFF00]">Techflow@outlook.fr</a></li>
              <li><a href="mailto:hola@techflow.es" className="hover:text-[#CCFF00]">hola@techflow.es</a></li>
              <li className="text-gray-500 text-sm mt-4">
                {isSpanish ? "Respuesta en 24h laborables." : "Réponse sous 24h ouvrables."}
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="border-t border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 font-mono text-sm text-gray-500">
          <p>© {new Date().getFullYear()} TechFlow Solutions. {isSpanish ? "Todos los derechos reservados." : "All rights reserved."}</p>
          <div className="flex gap-6">
            <Link href="/legal"><span className="hover:text-[#CCFF00] cursor-pointer">{t('footer.legal')}</span></Link>
            <a href="#" className="hover:text-[#CCFF00]">{t('footer.privacy')}</a>
          </div>
        </div>
      </div>
    </footer>
